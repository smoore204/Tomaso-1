...
namespace Tomaso
{
    public class DefineSlabSection: GH_Component
    {
        /// <summary>
        ...
        /// </summary>
        public DefineSlabSection()
          : base("Define > Slab > Slab Section", "DfnSlbSctn",
            "ETABSv1.SapModel.PropArea.SetSlab()\nInitializes a slab property",
            "Tomaso", "04_Define")
        {
        }
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            In_Name = pManager.AddTextParameter("Name", "N", "The name of a slab property. If this is an existing property, that property is modified; otherwise, a new property is added.",
                GH_ParamAccess.list);
            In_SlabType = pManager.AddIntegerParameter("SlabType", "S", "This is one of the items in the eSlabType enumeration.\n0. Slab \n1. Drop \n3. Ribbed \n4. Waffle \n5. Mat \n6. Footing \nIf this item is Ribbed, use the Define Slab Ribbed component to get additional parameters\nIf this item is Waffle, use the Assign Slab Waffle Component to get additional parameters",
                GH_ParamAccess.list);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetDataList(In_SlabType, SlabType)) { return; }
            ...
            if (Run)
            {
                ...
                if (SlabType.Count == 1)
                {
                    for (int i = 0; i < Name.Count - 1; i++)
                    {
                        SlabType.Add(SlabType[0]);
                    }
                }

                if (ShellType.Count == 1)
                {
                    for (int i = 0; i < Name.Count - 1; i++)
                    {
                        ShellType.Add(ShellType[0]);
                    }
                }
                ...
                for (int i = 0; i < Name.Count; i++)
                {
                    switch (SlabType[i])
                    {
                        ...
                        case 4:
                            ...
                            break;
                        case 5:
                            slabType = eSlabType.Mat;
                            break;
                        case 6:
                            slabType = eSlabType.Footing;
                            break;
                    }
                    ...
                    ret = mySapModel.PropArea.SetSlab(Name[i], slabType, shellType, MatProp[i], Thickness[i], Color[i], Notes[i], GUID[i]);
                }
        ...
        /// </summary>
        public override Guid ComponentGuid => new Guid("03c18746-b78e-419c-8ea4-7843ef7e2046");
    }
}