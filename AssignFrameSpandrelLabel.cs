...
namespace Tomaso
{
    public class AssignFrameSpandrelLabel: GH_Component
    {
        /// <summary>
        ...
        /// </summary>
        public AssignFrameSpandrelLabel()
          : base("Assign > Frame > Spandrel Label", "AssgnFrmSpndrlLbl",
            "ETABSv1.SapModel.FrameObj.SetSpandrel()\nSets the Spandrel label assignment of one or more frame objects ",
            "Tomaso", "06_Assign")
        {
        }
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            In_Name = pManager.AddTextParameter("Name", "N", "The name of an existing frame object or group depending on the value of the ItemType item.", 
                GH_ParamAccess.list);
            In_SpandrelName = pManager.AddTextParameter("SpandrelName", "S", "The name of the Spandrel assignment, or 'None', to unset any assignment",
                GH_ParamAccess.list);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<string> SpandrelName = new List<string>();
            int ItemType = 0;

            if (!DA.GetData(In_Run, ref Run)){return;}
            if (!DA.GetDataList(In_Name, Name)){ return; }
            if (!DA.GetDataList(In_SpandrelName, SpandrelName)) { return; }
            ...
            if (Run)
            {
                ...
                if (SpandrelName.Count == 1)
                {
                    for (int i = 0; i < Name.Count - 1; i++)
                    {
                        SpandrelName.Add(SpandrelName[0]);
                    }
                }
                ...
                {
                    ret = mySapModel.FrameObj.SetSpandrel(Name[i], SpandrelName[i], itemType);
        ...
        /// </summary>
        public override Guid ComponentGuid => new Guid("7edf6835-9daa-43fe-bd93-a68a99f5417a");
    }
}