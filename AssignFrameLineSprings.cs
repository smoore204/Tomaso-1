...
namespace Tomaso
{
    public class AssignFrameLineSprings: GH_Component
    {
        /// <summary>
        ...
        /// </summary>
        public AssignFrameLineSprings()
          : base("Assign > Frame > Line Springs", "AssgnFrmLnSprngs",
            "ETABSv1.SapModel.FrameObj.SetSpringAssignments()\nAssigns an existing named line spring property to frame objects.",
            "Tomaso", "06_Assign")
        {
        }
        ...
        protected override void RegisterInputParams(GH_Component.GH_InputParamManager pManager)
        {
            ...
            In_Name = pManager.AddTextParameter("Name", "N", "The name of an existing frame object or group depending on the value of the ItemType item.", 
                GH_ParamAccess.list);
            In_SpringProp = pManager.AddTextParameter("SpringProp", "S", "The name of an existing line spring property ",
                GH_ParamAccess.list);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<string> SpringProp = new List<string>();
            int ItemType = 0;

            if (!DA.GetData(In_Run, ref Run)){return;}
            if (!DA.GetDataList(In_Name, Name)){ return; }
            if (!DA.GetDataList(In_SpringProp, SpringProp)) { return; }
            ...
            if (Run)
            {
                ...
                if (SpringProp.Count == 1)
                {
                    for (int i = 0; i < Name.Count - 1; i++)
                    {
                        SpringProp.Add(SpringProp[0]);
                    }
                }
                ...
                {
                    ret = mySapModel.FrameObj.SetSpringAssignment(Name[i], SpringProp[i], itemType);
        ...
        protected override System.Drawing.Bitmap Icon { get { return Tomaso.Properties.Resources.AFrameLineSpring; } }
        ...
        /// </summary>
        public override Guid ComponentGuid => new Guid("ba962660-774c-429b-9dd2-8396527082c4");
    }
}